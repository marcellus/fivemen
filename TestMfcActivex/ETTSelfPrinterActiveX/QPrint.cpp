...
int CQPrint::Dialog (char *PrinterName, int Pages)
{
	...
	if (PrinterName == NULL) {
		...
		if (m_PrinterDC.CreateDC (NULL, PrinterName, NULL, NULL) == 0) {
			...
			CString str_ttt;
			str_ttt.Format("Γ»ΣΠΥÒµ½΄ςΣ΅»ϊ%s",PrinterName);
			AfxMessageBox(str_ttt,0,0);
...
void CQPrint::Print (HPRIVATEFONT font, CString strText, UINT Format, int left)
{
	...
	if (Size.cx > Width)
	{
		div_result = div(static_cast<int>(Size.cx), Width );
	...
	for (int i=0;i<nLoop;i++)
	{
		...
		if (nLoop > 1)
		{
			...
			//strText=strText.Mid(count,strText.GetLength()-count);

			//βρες αν υπάρχε?κενό μέχρ?MAXCHARS πριν. Αν να?τότε μείωσε ανάλογ?διαφορετικ?προχώρ?
		...
		if (!(Format & FORMAT_NOLF)) {
			...
			//CString strErrorID;
			//strErrorID.Format(_T("Φ΄ΠΠ»»ΠΠΈί¶ΘΌΖΛγ£΅ %u"),m_y);
			//AfxMessageBox((LPCTSTR)strErrorID,0,0);
