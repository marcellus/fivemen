...
//	email :	fuushikaden@yahoo.com
	...
	var imgDir = "../Js/PopcalendarImages/";			// directory for images ... e.g. var imgDir="/img/"
    ...
    */
		...
		if (!ns4)
		{
			...
			if (showToday==1)
			{
				document.getElementById("lblToday").innerHTML =	todayString + 
					" <a onmousemove='window.status=\""+gotoString+"\"' onmouseout='window.status=\"\"' title='"+gotoString+"' style='"+styleAnchor+"' href='javascript:monthSelected=monthNow;yearSelected=yearNow;constructCalendar();'>"+
					yearNow + "年" + monthNow + "月" + dateNow + "日，星期" + dayName[(today.getDay()-startAt==-1)?6:(today.getDay()-startAt)]
	...
	function closeCalendar() {
		...
		//objLastInput = ctlToPlaceValue;
		//ctlToPlaceValue.onblur();
	}
	
	//var objLastInput = null;
	...
	function popUpCalendar(ctl,	ctl2, format) 
	{
		...
		{ 
			...
			if ( crossobj.visibility ==	"hidden" ) {
				...
				var nTemp = fixedX==-1 ? ctl.offsetLeft	+ leftpos :	fixedX;
				var nWidth = 256;
				if (parseInt(nTemp) + nWidth > document.body.clientWidth)
				{
					nTemp = (document.body.clientWidth - nWidth) + "px";
				}
				crossobj.left =	nTemp;
