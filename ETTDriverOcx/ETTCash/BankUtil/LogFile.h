...
protected:
	...
	virtual void WriteLog( LPCVOID lpBuffer, DWORD dwLength)//写日志, 可以扩展修改
	{
		...
		if(IsOpen())
		{
			...
			WriteFile(_hFile, "-----------------------------#\xd\xa", 32, &dwWriteLength, NULL);
			//DWORD dwSize = 0;
			//DWORD dwLen = WideCharToMultiByte(CP_ACP, NULL, lpBuffer, -1, NULL, NULL, NULL, FALSE );
			//char* pchBuffer = new char[dwLen];
			//WideCharToMultiByte(CP_ACP, NULL, lpBuffer, -1, pchBuffer, dwLen, NULL, FALSE );
			//WriteFile(hFile, lpBuffer, dwLen+1, &dwSize, NULL );
			//delete[] pchBuffer;
...
public:
	...
	void SetFileName(const char *szName)//修改文件名， 同时关闭上一个日志文件
	{
		...
		_szFileName = new char[strlen(szName) + 128];
		assert(_szFileName);
...
protected:
	...
	void SetPath(const char *szPath)
	{
		...
		else
		{
			...
			_szPath = new char[strlen(temp) + 128];
			memset(_szPath,0,strlen(temp) + 128);
			assert(_szPath);
			strcpy(_szPath, temp);
		}
	}
