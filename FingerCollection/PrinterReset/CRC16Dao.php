...
*
*/
class Dao_Member_Engine_CRC16Dao{
	 ...
	 * @return unknown
	 */
	public function getCRC16Code($str)	{
		$CRC16_Lookup=self::getCrc16Table();
		$len = strlen($str);
		$crc=0;//short
		$high=0;//byte

		for($i = 0; $i < $len; $i++ )		{
			$high=$this->byte(($crc&0xffff)>>8);
			$crc=$this->short($crc<<8);
			$index=($high^ord($str[$i]))&0xff;
			$crc^=intval($CRC16_Lookup[$index]&0xffff);
			$crc=$this->short($crc);
		}
		return $crc;
	}
	...
	/**
	 * 模仿Java的强制类型转换，将一个数转换成Java byte型
	 *
	 * @param unknown_type $num
	 * @return unknown
	 */
	private function byte($num){
		$byte=$num&0xff;
	...
	/**
	 * 模仿Java的强制类型转换，将一个数转换成Java short型
	 *
	 * @param unknown_type $num
	 * @return unknown
	 */
	private function short($num){
		$short=$num&0xffff;
	 ...
	 * @return Array
	 */
	private static function getCrc16Table(){
