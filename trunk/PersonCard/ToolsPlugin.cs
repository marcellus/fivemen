...
namespace PersonCard
{
    [Plugin(ChangeLogPath = "Tools-ChangeLog.txt", Company = "Fight Together", Developer = "deadshot123",
      Email = "deadshot123@qq.com", MainVersion = "1.0", Name = "工具插件-Version2", Tel = "15814584509", Url = "http://deadshot123.cnblogs.com")]
    public class ToolsPlugin : AbstractWindowPlugin
    {
        public override void EmmitMenu()
        {
            ...
            tmp = this.BuildSubMenu("添加省份信息", typeof(FT.Windows.CommonsPlugin.ProvinceBrowser));
            top.DropDownItems.Add(tmp);
            tmp = this.BuildSubMenu("省份信息管理", typeof(FT.Windows.CommonsPlugin.ProvinceSearch));
            top.DropDownItems.Add(tmp);

            this.AddSeparatorToMenu(top);
            tmp = this.BuildSubMenu("添加市区信息", typeof(FT.Windows.CommonsPlugin.CityBrowser));
            top.DropDownItems.Add(tmp);
            tmp = this.BuildSubMenu("市区信息管理", typeof(FT.Windows.CommonsPlugin.CitySearch));
            top.DropDownItems.Add(tmp);

            this.AddSeparatorToMenu(top);
            tmp = this.BuildSubMenu("添加县市信息", typeof(FT.Windows.CommonsPlugin.AreaBrowser));
            top.DropDownItems.Add(tmp);
            tmp = this.BuildSubMenu("县市信息管理", typeof(FT.Windows.CommonsPlugin.AreaSearch));
            top.DropDownItems.Add(tmp);
        ...
        {
            this.AddTopTool(PersonCard.Resource.Config, "基础数据管理",typeof(FT.Windows.CommonsPlugin.DictSearch));
            this.AddTopTool(PersonCard.Resource.Notepad, "记事本").Click += new EventHandler(notepad_Click);
