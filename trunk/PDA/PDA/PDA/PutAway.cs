...
namespace PDA
{
    class PutAway : IMySerializable
    {
        ...
        public static string TempFileUrl
        {
            get
            {
                return DB.RootPath + @"BackUp\PutAway";
            }
        }
        ...
        public DataTable LoksDataSource
        {
            get
            {
                return lokDataSource;
            }
            set { lokDataSource = value; }
        }
        ...
        {
            this.skuDataSource = ds.Tables["PutAwayBill"];
            this.diskDataSource = ds.Tables["PutAwayDisk"];
            if (ds.Tables.Count > 2)
            {
                this.lokDataSource = ds.Tables[2];
            }
            else
            {
                CreateLokDataTable();
                ds.Tables.Add(this.lokDataSource);
            }
        }
        ...
        private SKU GetCurrentSKU(string skuId)
        {
            ...
            foreach (SKU s in skus.Values)
            {
                if (s.Id == skuId && s.Receiptkey == currentDisk.Receiptkey)// && s.Lot == currentDisk.Lot
                {
